...
<div class="wpaicg-alert">
    ...
    echo sprintf(
        ...
        esc_html__('You must configure a %1$sCron Job%2$s on your hosting/server. If this is not done, the Index Builder feature will not be available for use.', 'gpt3-ai-content-generator'),
        '<a href="https://www.hostgator.com/help/article/what-are-cron-jobs" target="_blank">',
        '</a>'
    );
    ?>
    </p>
    ...
    if(!empty($wpaicg_cron_job_last_time)):
        ...
        <p>
            <?php
            echo sprintf(
                /* translators: 1: date and time of last cron job, 2: time ago since last cron job */
                esc_html__('The last time, the Cron Job ran on your website %1$s (%2$s ago)', 'gpt3-ai-content-generator'),
                gmdate('Y-m-d H:i:s', $wpaicg_cron_job_last_time),
                $wpaicg_output
            );
            ?>
        </p>
    ...
    echo sprintf(
        ...
        esc_html__('If you are using a Linux/Unix server, copy the code below and paste it into the crontab. Read the detailed guide %1$shere%2$s.','gpt3-ai-content-generator'),
        '<a href="'.esc_url("https://docs.aipower.org/docs/AutoGPT/gpt-agents#cron-job-setup").'" target="_blank">',
        '</a>'
    ); 
    ?>
    </p>
...
echo sprintf(
    /* translators: 1: opening anchor tag, 2: closing anchor tag */
    esc_html__('Free users can only generate 5 pieces of content at a time. Please %1$sclick here%2$s to upgrade to the Pro plan to unlock more.', 'gpt3-ai-content-generator'),
    '<a href="'.admin_url('admin.php?page=wpaicg-pricing').'">',
    '</a>'
); 
?></div>
