...
if (isset($_GET['delete_all'])) {
    // Verify nonce for the delete all action
    if (!isset($_GET['wpaicg_delete_all_nonce_field']) || !wp_verify_nonce($_GET['wpaicg_delete_all_nonce_field'], 'wpaicg_delete_all_nonce')) {
        die(esc_html__('Nonce verification failed', 'gpt3-ai-content-generator'));
    }
    // SQL to get all IDs of posts with 'wpaicg_bulk' or 'wpaicg_tracking' post type
    $tasks_sql = $wpdb->prepare("SELECT ID FROM " . $wpdb->posts . " WHERE post_type IN (%s, %s)", 'wpaicg_bulk', 'wpaicg_tracking');
    $tasks = $wpdb->get_results($tasks_sql, ARRAY_A);

    // Loop through each task and delete it
    foreach ($tasks as $task) {
        $wpdb->delete($wpdb->posts, ['ID' => $task['ID']]);
    }
...
<form action="" method="get">
    ...
    <div class="wpaicg-d-flex mb-5">
        ...
        <?php wp_nonce_field('wpaicg_delete_all_nonce', 'wpaicg_delete_all_nonce_field'); ?>
        <button class="button wpaicg-danger-btn" type="submit" name="delete_all" onclick="return confirm('<?php echo esc_html__('This will delete all pending and completed tasks. However, the generated content will not be affected. Do you wish to proceed?','gpt3-ai-content-generator')?>')"><?php echo esc_html__('Delete All Tasks','gpt3-ai-content-generator')?></button>
    </div>
</form>
<?php
