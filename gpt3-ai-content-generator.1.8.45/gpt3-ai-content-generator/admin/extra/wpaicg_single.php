...
?>">
                ...
                <div class="nice-form-group">
                    <button type="button" name="get_preview" id="wpcgai_load_plugin_settings" class="button button-primary"><?php 
...
?></th>
                    <th><?php 
echo  esc_html__( 'Word Count', 'gpt3-ai-content-generator' ) ;
?></th>
                </tr>
                </thead>
                <tbody>
...
if ( $wpaicg_single_logs->have_posts() ) {
    foreach ( $wpaicg_single_logs->posts as $wpaicg_single_log ) {
        ...
        if ( isset( $wpaicg_usage_token ) && !empty($wpaicg_usage_token) && is_numeric( $wpaicg_usage_token ) ) {
            echo  esc_html( round( (double) $wpaicg_usage_token ) ) ;
        } else {
...
?></label>
                                    <textarea class="wpaicg_custom_template_prompt_content" name="template[prompt_content]" rows="5"><?php 
...
echo  sprintf(
    /* translators: 1: minimum max token value, 2: maximum max token value */
    esc_html__( 'For gpt-4-1106-preview, please enter a valid max token value between %1$d and %2$d.', 'gpt3-ai-content-generator' ),
    0,
    128000
) ;
?>';
